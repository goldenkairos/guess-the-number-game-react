...
function SecretNumber() {
  ...
  function updateBoard(userInput, currRowIdx) {
    const usetInputArr = userInput.split("");
    let count = 0;
    ...
    const validateUserGuess = usetInputArr.map((inputDigit, idx) => {
      if (inputDigit === secretNumber[idx]) {
        ...
        return { numer: inputDigit, status: "#373b37" }; //gray for incorrect input
      }
    });
    ...
    setBoard((prevBoard) => {
      ...
      updateBoard[currRowIdx] = validateUserGuess;
      return updatedBoard;
    });
  ...
  function checkGameCompletion(){
    ...
    if (numberGenerator == 4 || correctCount == 4){
      setIsGameCompleted(true);
    }
  }

  useEffect(() => {checkGameCompletion()}, [userGuesses]);
  ...
  return (
    ...
    userWin={userWin}/> : <div></div>
    }
      <UserGuess handleUserInput={handleUserInput} 
